...
public class CryptoService {
    ...
    private final FileHandler fileHandler; // Para lectura/escritura básica
    ...
    public void encryptFile(String inputFilePath, String outputFilePath, String password) throws Exception {
        ...
        // Escribir todo al archivo de salida usando ByteArrayOutputStream para el formato
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream();
             FileOutputStream fos = new FileOutputStream(outputFilePath)) { // O usar fileHandler.writeBytes(outputFilePath, bundledData);
            baos.write(salt);
            baos.write(iv);
            baos.write(originalHash);
            baos.write(encryptedBytes);
            fos.write(baos.toByteArray()); // Escribe el contenido del BAOS al archivo
        }
    }

    public boolean decryptFileAndVerify(String inputFilePath, String outputFilePath, String password) throws Exception {
        ...
        bis.read(iv);
        IvParameterSpec ivParameterSpec = new IvParameterSpec(iv);
        byte[] storedHash = new byte[32]; // CryptoConstants.HASH_LENGTH_BYTES si la defines
        bis.read(storedHash);
        ...
        } catch (Exception e) { // BadPaddingException, IllegalBlockSizeException
